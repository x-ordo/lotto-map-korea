...
async function main() {
    // We will use the data provided in the prompt directly as a fallback if CSV fails
    const promptData = `종합복권슈퍼,경기 시흥시 마유로 336,경기 시흥시 정왕동 1735-2번지,1등 당첨: 3회,3
  ...
  홀인원복권,대구 수성구 달구벌대로 3294 시지퍼스트,대구 수성구 신매동 267,1등 당첨: 1회,1`;
    ...
    const records = parse(promptData, {
        columns: false,
    ...
    for (const record of records) {
        const [name, address, oldAddress, description, winCount] = record;
        ...
        if (geo) {
            processedStores.push({
                ...
                lng: geo.lng,
                type: ['LOTTO'],
                winCount1st: parseInt(winCount, 10) || 0,
                winCount2nd: 0,
                description
            });
            console.log(`Geocoded: ${name}`);
        } else {
            console.warn(`Failed to geocode: ${name} (${address})`);
        }
...
`;
    ...
    console.log('Successfully saved to lib/data.ts');
}
